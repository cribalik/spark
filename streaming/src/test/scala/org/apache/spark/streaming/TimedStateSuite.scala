...
import org.apache.spark.streaming.dstream.{DStream, InternalTimedStateDStream,
  TimedStateDStream, TimedStateDStreamImpl}
import org.apache.spark.util.{ManualClock, Utils}

class TimedStateSuite extends SparkFunSuite
with DStreamCheckpointTester with BeforeAndAfterAll with BeforeAndAfter {
  ...
  override def beforeAll(): Unit = {
    super.beforeAll()
    val conf = new SparkConf().setMaster("local").setAppName("TimedStateSuite")
  ...
  test("timedState - basic operations with simple API") {
    val inputData =
      Seq(
        Seq(),
        Seq("a"),
    ...
    val mappingFunc = (key: String, value: Option[Int], state: TimedState[Int]) => {
    ...
    testOperation[String, Int, Int](
      inputData, TimedStateSpec.function(mappingFunc), outputData, stateData)
  }

  test("timedState - basic operations with advanced API") {
    val inputData =
      Seq(
        Seq(),
        Seq("a"),
    ...
    val mappingFunc =
      (batchTime: Time, key: String, value: Option[Int], state: TimedState[Int]) => {
      val sum = value.getOrElse(0) + state.getOption.getOrElse(0)
      state.update(sum)
      Some(key * 2)
    }

    testOperation(inputData, TimedStateSpec.function(mappingFunc), outputData, stateData)
  }

  test("timedState - type inferencing and class tags") {

    // Simple track state function with value as Int, state as Double and mapped type as Double
    val simpleFunc = (key: String, value: Option[Int], state: TimedState[Double]) => {
      0L
    }

    // Advanced track state function with key as String, value as Int, state as Double and
    // mapped type as Double
    val advancedFunc = (time: Time, key: String, value: Option[Int], state: TimedState[Double]) => {
      Some(0L)
    }

    def testTypes(dstream: TimedStateDStream[_, _, _, _]): Unit = {
      val dstreamImpl = dstream.asInstanceOf[TimedStateDStreamImpl[_, _, _, _]]
    ...
    // Defining TimedStateSpec inline with timedState and simple function implicitly gets the types
    val simpleFunctionStateStream1 = inputStream.timedState(
      TimedStateSpec.function(simpleFunc).numPartitions(1))
    testTypes(simpleFunctionStateStream1)

    // Separately defining TimedStateSpec with simple function requires explicitly specifying types
    val simpleFuncSpec = TimedStateSpec.function[String, Int, Double, Long](simpleFunc)
    val simpleFunctionStateStream2 = inputStream.timedState(simpleFuncSpec)
    testTypes(simpleFunctionStateStream2)

    // Separately defining TimedStateSpec with advanced function implicitly gets the types
    val advFuncSpec1 = TimedStateSpec.function(advancedFunc)
    val advFunctionStateStream1 = inputStream.timedState(advFuncSpec1)
    testTypes(advFunctionStateStream1)

    // Defining TimedStateSpec inline with timedState and advanced func implicitly gets the types
    val advFunctionStateStream2 = inputStream.timedState(
      TimedStateSpec.function(simpleFunc).numPartitions(1))
    testTypes(advFunctionStateStream2)

    // Defining TimedStateSpec inline with timedState and advanced func implicitly gets the types
    val advFuncSpec2 = TimedStateSpec.function[String, Int, Double, Long](advancedFunc)
    val advFunctionStateStream3 = inputStream.timedState[Double, Long](advFuncSpec2)
    testTypes(advFunctionStateStream3)
  }

  test("timedState - states as mapped data") {
    val inputData =
      Seq(
        Seq(),
        Seq("a"),
    ...
    val mappingFunc = (time: Time, key: String, value: Option[Int], state: TimedState[Int]) => {
      val sum = value.getOrElse(0) + state.getOption.getOrElse(0)
      val output = (key, sum)
      state.update(sum)
      Some(output)
  ...
  test("timedState - initial states, with nothing returned as from mapping function") {
    ...
    val timedTimedStateSpec =
      TimedStateSpec.function(mappingFunc).initialState(sc.makeRDD(initialState))
    testOperation(inputData, timedTimedStateSpec, outputData, stateData)
  }

  test("timedState - state removing") {
    val inputData =
      Seq(
        Seq(),
        Seq("a"),
    ...
    testOperation(
      inputData, TimedStateSpec.function(mappingFunc).numPartitions(1), outputData, stateData)
  }

  test("timedState - state timing out") {
    val inputData =
      Seq(
        Seq("a", "b", "c"),
        Seq("a", "b"),
    ...
    val (collectedOutputs, collectedStateSnapshots) = getOperationOutput(
      inputData, TimedStateSpec.function(mappingFunc), 20)
  ...
  test("timedState - checkpoint durations") {
    val privateMethod = PrivateMethod[InternalTimedStateDStream[_, _, _, _]]('internalStream)
    ...
    def testCheckpointDuration(
      ...
      try {
        ...
        val dummyFunc = (key: Int, value: Option[Int], state: TimedState[Int]) => 0
        val timedStateStream = inputStream.timedState(TimedStateSpec.function(dummyFunc))
        val internaltimedStateStream = timedStateStream invokePrivate privateMethod()

        explicitCheckpointDuration.foreach { d =>
          timedStateStream.checkpoint(d)
        }
        timedStateStream.register()
        ssc.checkpoint(checkpointDir.toString)
        ssc.start()  // should initialize all the checkpoint durations
        assert(timedStateStream.checkpointDuration === null)
        assert(internaltimedStateStream.checkpointDuration === expectedCheckpointDuration)
      } finally {
  ...
  test("timedState - driver failure recovery") {
    val inputData =
      Seq(
        Seq(),
        Seq("a"),
    ...
    def operation(dstream: DStream[String]): DStream[(String, Int)] = {
      ...
      val runningCount = (key: String, value: Option[Int], state: TimedState[Int]) => {
        state.update(state.getOption().getOrElse(0) + value.getOrElse(0))
        state.get()
      }

      val timedStateStream = dstream.map { _ -> 1 }.timedState(
        TimedStateSpec.function(runningCount))
      // Set interval make sure there is one RDD checkpointing
      timedStateStream.checkpoint(checkpointDuration)
      timedStateStream.stateSnapshots()
    }
  ...
  // TODO: Test timeoutpass duration

  private def testOperation[K: ClassTag, S: ClassTag, T: ClassTag](
    input: Seq[Seq[K]],
    timedTimedStateSpec: TimedStateSpec[K, Int, S, T],
    ...
    getOperationOutput(input, timedTimedStateSpec, expectedOutputs.size)
  ...
  private def getOperationOutput[K: ClassTag, S: ClassTag, T: ClassTag](
    ...
    val trackeStateStream = inputStream.map(x => (x, 1)).timedState(timedTimedStateSpec)
